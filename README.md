...
4. **イベント駆動の速報 BUY 通知（任意）**<br>
   ...
   `DASHBOARD_PUBLIC_URL` を設定している場合は、 同じ Embed に「📊 dashboard」 リンクボタンも付与され、 該当 mint を選択した状態で公開ダッシュボードを直接開ける。<br>
...
5. **公開ダッシュボード（任意）**<br>
   `sm_signal_events` を期間別 (1h / 6h / 24h / 7d) に集計した JSON を別の public リポへ自動 push し、 GitHub Pages 上でブラウザから閲覧できる静的サイト。
   - 銘柄一覧 + **DexScreener / Birdeye のチャート切替 iframe** + buyer 内訳 (ウォレット / ラベル / 取引数 / 合計 / 最終取引時刻) を 1 ページで表示。
   - 期間タブ / ソート (買い手数・直近・SOL 合計・USD 合計・取引数) / 検索 / Min buyers をクライアント側で操作。 ペイン幅はドラッグで変更可、 スマホは一覧 ↔ 詳細の画面切替式。
   - cron を使わず **bot プロセス内 cog** が 5 分間隔で集計 → `git add / commit / push` を実行。 差分が無い周期はスキップするので無駄 commit は出ない。
   - 速報 BUY 通知の「📊 dashboard」 ボタンから `?mint=<CA>` 形式でその銘柄を自動選択した状態で開ける。
   - 集計閾値 (`DASHBOARD_MIN_DISTINCT_BUYERS` / `DASHBOARD_TOP_N` 等) と「速報通知の閾値」 は **完全に独立** で、 通知に出なかった低シグナル銘柄も含めて閲覧可能。
   - セットアップ手順とチューニング例は [`dashboard/README.md`](dashboard/README.md) 参照。
...
5. **公開ダッシュボード**<br>
   ローカルの `sm_signal_events` を集計するだけ。 symbol/mcap 補完も共有の DexScreener キャッシュ経由で Nansen は呼ばない。<br>
   コスト：0クレジット
...
[Helius] ── enhanced webhook POST ──→ aiohttp.web (sm_signal cog)
                                          ...
                                          └─ BUY hook → sm_summary.notify_realtime
                                                ...
                                                └─ SM_SUMMARY_CHANNEL_ID へ
                                                   「🚨 速報 BUY」 Embed
                                                   (+ /analyze / 📊 dashboard ボタン)
                                                                                    │
[dashboard_publisher cog] ── 5 分間隔 ─────────────────────────────────────────────┘
   ├─ sm_signal_events を 1h/6h/24h/7d に集計 → JSON
   ├─ <DASHBOARD_REPO_PATH>/data/ に書き出し
   └─ 差分があれば git add / commit / push
            │
            ▼
   GitHub Pages (別 public リポ) で銘柄一覧 + チャート iframe を公開
```
