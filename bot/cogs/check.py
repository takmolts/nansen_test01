...
_LLM_SYS_PROMPT = (
    ...
    "渡されたデータを読み解き、 日本語の総括コメントを書け。\n"
    "\n"
    "【出力フォーマット】\n"
    "**結論**: 全体評価を 1〜2 文 (総合スコアの理由を本文で説明)\n"
    "**注目点**:\n"
    "- 強み1: 数値を本文に引用しつつ 1〜2 文で説明\n"
    "- 強み2: 同上\n"
    "**注意点**:\n"
    "- リスク1: 数値を本文に引用しつつ 1〜2 文で説明\n"
    "- リスク2: 同上\n"
    "\n"
    "【厳守ルール】\n"
    "- キーワード単発禁止。 例: 「高リスク」 のみは NG → 「Top10 集中度 42% で whale 寄り、 急落リスクあり」 のように数値と理由を含める\n"
    "- 渡された数値 (mcap / liquidity / 各カテゴリスコア / breakdown) を必ず本文に引用する\n"
    "- 投資推奨表現 (「買い時」 「おすすめ」 「狙い目」 等) は禁止。 観察コメントのみ\n"
    "- 前置き・後書き・自己紹介・メタ発言禁止\n"
    "- 全体で 1000 文字以内に収めること"
)
